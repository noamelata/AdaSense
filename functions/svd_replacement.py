...
from models.mri_utils import fft2c_new, ifft2c_new
...
class GeneralH(H_functions):
    ...
    def singulars(self):
        return torch.ones_like(self.h)[:, 0]

    def H(self, v):
        return (self.h.to(v.device) @ v.reshape(v.shape[0], -1, 1)).reshape(v.shape[0], -1)

    def Ht(self, vec):
        return (self.h.to(vec.device).t() @ vec.reshape(vec.shape[0], -1, 1)).reshape(vec.shape[0], -1)

    def H_pinv(self, vec):
        return self.Ht(vec)

class MRIInpainting(H_functions):
    def __init__(self, mask, device, shape):
        self.c, self.h, self.w = shape
        self.mask = mask.to(device)

    def H(self, v):
        dft_v = fft2c_new(v.reshape(-1, self.c, self.h, self.w).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        return dft_v.reshape(v.shape[0], -1)[:, self.mask.to(v.device)]

    def Ht(self, vec):
        v = torch.zeros(vec.shape[0], self.c * self.h * self.w, device=vec.device)
        v[:, self.mask.to(vec.device)] = vec
        return ifft2c_new(v.reshape(-1, self.c, self.h, self.w).permute(0, 2, 3, 1)).permute(0, 3, 1, 2).reshape(v.shape[0], -1)

    def H_pinv(self, vec):
        return self.Ht(vec)

class CTRows(H_functions):
    def __init__(self, angles, device, shape):
        self.c, self.h, self.w = shape
        self.angles = angles.to(device)
        self.rows = torch.zeros((0, (self.c * self.h * self.w)), device=device)
        for i in self.angles:
            row = torch.from_numpy(np.load(os.path.join(f"CT_radon", f"CT_H_{i}.npy"))).to(self.angles.device)
            self.rows = torch.cat([self.rows, row], dim=0)
        self.h_pinv = torch.linalg.pinv(self.rows).to(self.angles.device)

    def add_rows(self, angles):
        self.angles = torch.cat([self.angles, angles.to(self.angles.device)], dim=0)
        for i in angles:
            row = torch.from_numpy(np.load(os.path.join(f"CT_radon", f"CT_H_{i}.npy"))).to(self.angles.device)
            self.rows = torch.cat([self.rows, row], dim=0)
        self.h_pinv = torch.linalg.pinv(self.rows).to(self.angles.device)
    ...
    def H(self, v):
        return (self.rows @ v.reshape((v.shape[0], (self.c * self.h * self.w), 1))).squeeze(-1)

    def Ht(self, vec):
        return (self.rows.t().unsqueeze(0) @ vec.unsqueeze(-1)).reshape((vec.shape[0], self.c, self.h, self.w))

    def H_pinv(self, vec):
        return (self.h_pinv.unsqueeze(0) @ vec.unsqueeze(-1)).reshape((vec.shape[0], self.c, self.h, self.w))
